...
class GraphFormerEncoder(nn.Module):
    ...
    def forward(self, x_in, x_in_k=None, x_in_v=None, mask=None, mask_fixer=None):
        """
        Args:
        ...
        for layer in self.layers:
            if x_in_k is not None and x_in_v is not None:
                x = layer(x, x_k, x_v, mask, mask_fixer)
            else:
                x = layer(x, mask=mask, mask_fixer=mask_fixer)
...
class GraphFormerEncoderLayer(nn.Module):
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def forward(self, x, x_k=None, x_v=None, mask=None, mask_fixer=None):
        """
        Args:
        ...
        if x_k is None and x_v is None:
            x, _ = self.self_attn(query_nodes=x, key_nodes=x, value_nodes=x, 
                                  edge_mask=mask, mask_fixer=mask_fixer)
        else:
            x_k = self.maybe_layer_norm(0, x_k, before=True)
            x_v = self.maybe_layer_norm(0, x_v, before=True) 
            x, _ = self.self_attn(query_nodes=x, key_nodes=x_k, value_nodes=x_v, 
                                  edge_mask=mask, mask_fixer=mask_fixer)
