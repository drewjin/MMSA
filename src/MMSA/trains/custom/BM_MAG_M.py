...
class BM_MAG_M():
    def __init__(self, args, trainer=None):
        ...
        if not args.use_accelerate:
            ...
            self.tokenizer = AutoTokenizer.from_pretrained(args.weight_dir)
        else:
            ...
            self.model, self.data_loader, self.criterion = accelerator.prepare(
                model, data_loader, self.criterion
            )
    
    def get_model_dataloader(self):
        return self.model, self.data_loader

    def get_mamba_token_ids(self, batch_data):
        bert_data = batch_data['text']
        raw_text = batch_data['raw_text']
        token_ids = self.tokenizer(
            raw_text,
            padding='max_length',
            max_length=bert_data.shape[-1],
            truncation=True,
            return_tensors='pt'
        ).input_ids
        return token_ids
        ...
        while True: 
            ...
            with tqdm(dataloader['train']) as td:
                for batch_data in td:
                    ...
                    mamba_token_ids = self.get_mamba_token_ids(batch_data).to(self.args.device)
                    bert_token_ids = batch_data['text'].to(self.args.device)
                    ...
                    optimizer.zero_grad()
                    # forward
                    outputs = model((mamba_token_ids, bert_token_ids), audio, vision)['M']
                    # compute loss
            ...
            with tqdm(dataloader) as td:
                for batch_data in td:
                    ...
                    if self.args.train_mode == 'classification':
                        ...
                        labels = labels.view(-1, 1)
                    outputs = model((mamba_token_ids, bert_token_ids), audio, vision)
